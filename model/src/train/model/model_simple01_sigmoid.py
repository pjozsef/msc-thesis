...
def create_model():
    ...
    conv1 = conv2d_relu(x, [3, 3, 1, 16], "conv1", "VALID")
    conv2 = conv2d_relu(conv1, [3, 3, 16, 16], "conv2", "VALID")
    pool1 = maxpool(conv2)
    conv3 = conv2d_relu(pool1, [5, 5, 16, 32], "conv3", "VALID")
    conv4 = conv2d_relu(conv3, [5, 5, 32, 32], "conv4", "VALID")
    pool2 = maxpool(conv4, 2)
    conv5 = conv2d_relu(pool2, [4, 4, 32, 32], "conv5", "VALID")
    pool3 = maxpool(conv5)
    ...
    d_conv5 = deconv2d_relu(d_pool3, "conv5", [tf.shape(x)[0], 195, 4, 32], "VALID")
    d_pool2 = upsample(d_conv5, 2)
    d_conv4 = deconv2d_relu(d_pool2, "conv4", [tf.shape(x)[0], 394, 12, 32], "VALID")
    d_conv3 = deconv2d_relu(d_conv4, "conv3", [tf.shape(x)[0], 398, 16, 16], "VALID")
    d_pool1 = upsample(d_conv3)
    d_conv2 = deconv2d_relu(d_pool1, "conv2", [tf.shape(x)[0], 798, 18, 16], "VALID")
    y = deconv2d_relu(d_conv2, "conv1", [tf.shape(x)[0], 800, 20, 1], "VALID")
